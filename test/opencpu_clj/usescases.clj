...
(fact "can use clojure.matrix dataset as inline parameter - 1"
  (let [ds (ds/row-maps mydat)]
    (:body (call-function-json-RPC server-url "base" "dim" {:x (j ds)})) => [3 3]))


(fact "can use clojure.matrix dataset as inline parameter - 2"
      (let [mydat-json (ds/row-maps mydat)]
        (shape (json-to-ds (:body (call-function-json-RPC server-url "stats" "reshape"
                                                          {:data (j mydat-json)
                                                           :varying  (j [2,3])
                                                           :v.names  (j "gdp")
                                                           :direction (j "long")
...
(fact "can send matrix to R and calculate eigen values with R"
      (let [m (matrix [[13 2][5 4]])]
        (:body (call-function-json-RPC server-url "base" "eigen" {:x (json/write-str m)})))
           => {:values [14 3], :vectors [[0.8944 -0.1961] [0.4472 0.9806]]})



(fact "can call lm and get data back"
      (let [fit (:body (call-function server-url "stats" "lm" {:formula "mpg~wt" :data "mtcars"}))
            coef (:body (call-function-json-RPC server-url "stats" "coefficients" {:object fit}))
            residuals (:body (call-function-json-RPC server-url "stats" "residuals" {:object fit}))
            fitted (:body (call-function-json-RPC server-url "stats" "fitted" {:object fit}))]
        {:coef coef
        :residuals residuals
        :fitted fitted}
...
(fact "can evaluate larger pieces of R code and get variables back as clojure data sets"
      (let [session-key (:body (call-function server-url "evaluate" "evaluate"
                                              {:input "
                                               ...
                                               "}))
            training (json-to-ds (:body (session-data server-url session-key "R/training" :json)))
            testing (json-to-ds (:body (session-data server-url session-key "R/testing" :json)))
            ]
...
(fact "can work woth data sets on server"
      (let [session-key-spam (:body (call-function "http://public.opencpu.org" "base" "identity" {:x "data(spam,package=\"kernlab\");spam"}))]
        (:body (call-function-json-RPC "http://public.opencpu.org" "base" "dim" {:x session-key-spam}))) => [4601,58])
