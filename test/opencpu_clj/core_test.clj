...
(fact "Can get error on retrieve non existing datasets"
       (get-dataset server-url "MASS" "xxx")=> {:result "data set 'xxx' not found" :status 400})
...
(fact "can call function which returns a S3 class"
      (count (:result (call-function server-url "utils" "sessionInfo" {} ))) => 15)
...
(fact "can pass unquoted symbols"
      (count (:result (call-function server-url "stats" "lm" {:formula "mpg~am" :data "mtcars"}))) => 15)
...
(fact "can get error on eval-R"
   ...
   {:result  "object 'xxxx' not found\n\nIn call:\nparse_all(input, filename, stop_on_error != 2L)\n"
     :status 400})
