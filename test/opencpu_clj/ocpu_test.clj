...
(fact "can call function which returns session"
      (:result (object server-url "base" :R "seq" {:from 1 :to 5})) => (n-of anything 6))
...
(fact "can access session path as json"
      (let [first-key-path (first (:result (object server-url "base" :R "seq" {:from 1 :to 5})))]
        (:result (session server-url first-key-path :json)) => [1 2 3 4 5]))

(fact "can access session path, which is a dataframe. "
      (let [first-key-path (first (:result (object server-url "base" :R "data.frame"  {:gender "c(\"M\",\"M\",\"F\")" :ht "c(172,186.5,165)" :wt "c(91,99,74)"})))]
